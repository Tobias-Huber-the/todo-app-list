...
2. En la carpeta del proyecto ejecutar en una terminal:

   - `npm install`
   - `npm run dev`

3. Abrir en el navegador la URL que muestra Vite (por defecto `http://localhost:5173`).
...
- Antes de renderizar la lista, aplico el filtro sobre `tasks`:
  ...
  `const filteredTasks = tasks.filter((task) => { if (filter === 'active') return !task.done; if (filter === 'done') return task.done; return true; })`
...
1. Usa `useState` para crear el estado.
2. En la función inicial del estado intenta leer un valor previo desde `localStorage`.
3. Usa `useEffect` para escribir en `localStorage` cada vez que el valor cambia.
